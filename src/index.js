...
import {did_authn, sign_message, get_aes_private_key_encrypted, create_vault_access, transfer_to, destroy_workspace, ownerstocontracts} from "./talao_transaction.js";
...
async function getdidauthn(){
  ...
  const data = await did_authn(provider, web3);
  const wc = await ownerstocontracts(provider.accounts[0],web3);
  const did = "did:talao:talaonet:" + wc.substr(2);
